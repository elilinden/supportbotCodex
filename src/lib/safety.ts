...
const DANGER_PATTERNS: DangerPattern[] = [
  ...
  { pattern: /(?:can'?t|cannot|could\s*n[o']?t)\s+(?:breathe|breath)/i, severity: "critical", category: "strangulation" },
  { pattern: /(?:wouldn'?t|would\s+not|didn'?t|did\s+not)\s+let\s+me\s+breathe/i, severity: "critical", category: "strangulation" },
  { pattern: /push(?:ed)?\s+(?:my|his|her)\s+(?:head|face)\s+(?:into|in)\s+(?:the\s+)?(?:pillow|mattress|cushion|couch)/i, severity: "critical", category: "strangulation" },
  { pattern: /compress(?:ed|ing)?\s+(?:my|his|her)\s+(?:throat|neck|airway)/i, severity: "critical", category: "strangulation" },
  { pattern: /held\s+(?:a\s+)?(?:pillow|cushion)\s+(?:over|on)\s+(?:my|his|her)\s+(?:face|head|mouth)/i, severity: "critical", category: "strangulation" },
  { pattern: /cover(?:ed|ing)?\s+(?:my|his|her)\s+(?:mouth|nose)\s+(?:and|&)\s+(?:nose|mouth)/i, severity: "critical", category: "strangulation" },
  { pattern: /pressure\s+on\s+(?:my|his|her)\s+(?:neck|throat|chest)/i, severity: "critical", category: "strangulation" },
  { pattern: /(?:couldn'?t|could\s+not)\s+(?:get\s+)?(?:air|oxygen)/i, severity: "critical", category: "strangulation" },
  { pattern: /(?:head|face)\s+(?:under|in)\s+(?:the\s+)?water/i, severity: "critical", category: "strangulation" },
  ...
  { pattern: /(?:can'?t|cannot)\s+stay\s+safe/i, severity: "high", category: "unsafe" },
  { pattern: /not\s+safe|(?:don'?t|do\s+not)\s+feel\s+safe|afraid\s+to\s+go\s+home/i, severity: "high", category: "unsafe" },
  ...
  { pattern: /(?:isolat|prevent|forbid|(?:won'?t|would\s*n[o']?t|will\s+not)\s+let)\s+(?:me|him|her)\s+(?:from\s+)?(?:see|leav|go|talk|contact|call)/i, severity: "high", category: "isolation" },
  { pattern: /took\s+(?:my|the)\s+(?:phone|keys|car|money|passport|documents)/i, severity: "high", category: "coercive_control" },
  { pattern: /control(?:s|led|ling)?\s+(?:the|my|our|all)\s+(?:money|finances|bank|account|credit\s+card)/i, severity: "high", category: "economic_control" },
  { pattern: /(?:won'?t|wouldn'?t|doesn'?t|didn'?t)\s+(?:let|allow)\s+(?:me|him|her)\s+(?:to\s+)?(?:work|get\s+a\s+job|have\s+(?:my|a)\s+(?:own\s+)?(?:money|account))/i, severity: "high", category: "economic_control" },
  { pattern: /(?:gives?|gave|hands?)\s+(?:me|him|her)\s+(?:an\s+)?allowance/i, severity: "high", category: "economic_control" },
  { pattern: /(?:hid(?:es?|ing)?|destroy(?:ed|ing)?)\s+(?:my|the)\s+(?:medication|pills|prescriptions?|medical\s+records?)/i, severity: "high", category: "medical_control" },
  { pattern: /(?:won'?t|wouldn'?t|doesn'?t|didn'?t)\s+(?:let|allow)\s+(?:me|him|her)\s+(?:to\s+)?(?:go\s+to\s+(?:the\s+)?(?:doctor|hospital|clinic)|take\s+(?:my\s+)?(?:medication|medicine))/i, severity: "high", category: "medical_control" },
  { pattern: /makes?\s+(?:all|every)\s+(?:the\s+)?decision/i, severity: "high", category: "decision_control" },
  { pattern: /(?:tells?|told)\s+(?:me|him|her)\s+(?:what\s+to\s+(?:wear|eat|say|do|think)|who\s+(?:I|to)\s+(?:can|could|should)\s+(?:talk|speak|be\s+friends)\s+(?:to|with))/i, severity: "high", category: "decision_control" },
  { pattern: /check(?:s|ed|ing)?\s+(?:my|the)\s+(?:mileage|odometer|receipts?|browser|history|email)/i, severity: "high", category: "surveillance" },
